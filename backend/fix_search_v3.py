...
def main():
    ...
    # 1. Fix InvoicesTab
    old_inv = '''<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
    ...
    new_inv = '''<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        <h2 className="text-xl font-bold text-slate-800">All Invoices</h2>
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
    ...
    # 2. Fix PaymentsTab
    # It seems PaymentsTab replacement worked mostly but let's check.
    # Actually, I used a regex for PaymentsTab signature that might have messed up the types.
    
    # 3. Fix CollectionsTab
    # I'll just search for the broken block in CollectionsTab.
