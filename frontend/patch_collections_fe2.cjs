...
// 5. Update form UI
content = content.replace(
  /<div>\s*<label className="block text-sm font-medium text-slate-700 mb-1">Received Date<\/label>\s*<input required type="date" value={receivedDate} onChange={e => setReceivedDate\(e\.target\.value\)} className="w-full border border-slate-300 rounded-lg p-2 outline-none focus:ring-2 focus:ring-sky-500" \/>\s*<\/div>/,
  `{isExpected ? (
              <div>
